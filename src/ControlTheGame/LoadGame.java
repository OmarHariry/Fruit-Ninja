...
public 	LoadGame(GameMode game) {
    ...
    public void loadProgress() {
        try {
            File inputFile = new File("src/Game.xml");
            ...
            NodeList kkList =doc.getElementsByTagName("Combo");
            ...
            for (int temp = 0; temp < kkList.getLength(); temp++) {
...
//                boat.setNoSails(Integer.valueOf(eElement.getElementsByTagName("HighScore").item(0).getTextContent()));
                ...
                eElement.getElementsByTagName("Combo").item(0).getTextContent();
                game.setCombo(Integer.parseInt(eElement.getElementsByTagName("Combo").item(0).getTextContent()));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
